...
def update_task_status(task_id: int, new_status: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    task = service.update_task_status(db, task_id, new_status, current_user.id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    return task

@router.post("/{task_id}/comments", response_model=schemas.TaskCommentResponse, status_code=status.HTTP_201_CREATED)
